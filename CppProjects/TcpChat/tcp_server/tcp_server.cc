...
void TcpServer::Stop() {
  is_stop_ = true;
  LOG << "TcpServer is going to quit, please wait";
  epoll_thread_.join();
  LOG << "TcpServer quit successfully";
}
...
void TcpServer::CreateEpoll() {
  ...
  if (epoll_fd_ == -1) {
    ELOG << "epoll_create() fail: " << std::strerror(errno);
    exit(EXIT_FAILURE);
  }
}
...
void TcpServer::AddListenSocketToEpoll() {
  ...
  if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, listen_sockfd_, &ev) == -1) {
    ELOG << "epoll_ctl() fail: " << std::strerror(errno);
    exit(EXIT_FAILURE);
  }
}
...
void TcpServer::HandleEpollEvent() {
  auto accept_event_handler = [this]() {
    ...
    if (conn_fd == -1) {
      ELOG << "accept() fail: " << std::strerror(errno);
      exit(EXIT_FAILURE);
    }
  ...
  while (!is_stop_) {
    ...
    if (fd_cnt == -1) {
      ELOG << "epoll_wait() fail: " << std::strerror(errno);
      exit(EXIT_FAILURE);
    }
