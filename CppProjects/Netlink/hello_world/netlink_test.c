...
int send_usrmsg(char* pbuf, uint16_t len) {
  /*
   * sk_buff: Linux 内核中用于表示网络数据包的数据结构, 用于在网络中传递和处理数据包
   *
   * @field: struct sk_buff *next 和 struct sk_buff *prev 构造了一个双向链表, 通常用于实现队列
   * @field: struct sk_buff *prev 指向与该数据包相关的 socket 指针, 以便内核根据 socket
   * 信息来处理数据包
   * @field: unsigned char *head 和 unsigned char *data 指向数据包的起始位置和实际数据内容起始地址
   * @field: unsigned int len 是 sk_buffer 中有效数据的长度
   * @field: nsigned int truesize 表示分配给 sk_buff 结构体的内存大小, 通常比 len
   * 大以容纳额外的元数据和内核信息
   * @field: struct skb_shared_info *shinfo 指向共享信息结构的指针,
   * 该结构用于存储与数据包相关的元信息, 如协议头和接口索引等信息
   * @field: struct sk_buff_head queue_mapping 用于将 sk_buff 结构体组织成队列
   * @field: unsigned char cb[48] 存储内核或网络协议栈的私有数据,
   * 各个网络协议栈的实现可以使用这个字段来存储自定义信息
   */
   ...
   *
   * struct nlmsghdr *nlmsg_put(struct sk_buff *skb, u32 pid, u32 seq, int type, int payload, int
   * flags);
   *
  ...
  /*
   * netlink_unicast: 向单个 Netlink socket 发送消息的函数, 用于将 Netlink 消息发送给指定的接收方
   * (通常是用户空间进程或者内核模块), 并且提供了一种简单的单播 (unicast) 机制, 实现了点对点的通信
   *
   * int netlink_unicast(struct sock *sk, struct sk_buff *skb, __u32 pid, int nonblock);
   *
   * @field: sk 指向要发送消息的 Netlink socket 指针, 这个 socket 是消息的发送方,
   * 用于指示消息应该通过哪个 Netlink socket 发送
   * @field: skb 指向包含要发送的 Netlink 消息的 sk_buff 结构体指针, 它包含了消息的数据和消息头
   * @field: pid 消息的接受者的用户空间进程 ID, 或者在内核模块之间通信时是接收模块的 PID,
   * 接收方通过这个 PID 确定消息的发送者
   * @field: nonblock 整数标志, 用于指示发送操作是否应该是非阻塞的, 如果将此参数设置为非零值 (一般是
   * MSG_DONTWAIT ) 那么发送操作是非阻塞的, 如果发送缓冲区已满则立即返回错误
   */
