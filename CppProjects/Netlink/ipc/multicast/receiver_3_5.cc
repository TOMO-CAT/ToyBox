...
int main() {
  ...
  src_addr.nl_groups = (1 << 3) | (1 << 5);  // specify the address which the process want to
                                             // receive if two or more address then turn on those
                                             // bits subscribe to 3 and 5 multicast address
  printf("Listeing to MCAST address (3 and 5): %d\n", src_addr.nl_groups);
